...
if (Ab_UpdateManager::$isCoreInstall){ 
	...
	if (!empty($modEShopCart)){
		...
		EShopCartManager::$instance->RolesDisable();
	
		$manCart = EShopCartManager::$instance;
	
		$manCart->PaymentSave(array(
            "tl" => $ph['cart_delivery_nal'],
            "def" => 1
        ));

		$d = new stdClass();
		$manCart->PaymentSave(array(
            "tl" => $ph['cart_delivery_bank']
        ));
	}
    ...
    SystemManager::$instance->RolesDisable();
	SystemManager::$instance->GetAdminManager()->CoreConfigSave(array(
        "style" => $ph['template'],
        "site_name" => $ph['sitename'],
        "site_title" => $ph['sitetitle']
    ));
